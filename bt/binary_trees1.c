...
void print_level(asciinode *node, int x, int level) 
{
  ...
  if (level == 0) 
  {
	  for (i=0; i<(x-print_next-((node->lablen-isleft)/2)); i++) 
    {
	  ...
	  print_next += i;
	  printf(GREEN "%s" RESETCOLOR, node->label);
...
void main() 
{
  ...
  while( i++ < 3 ) {
    int ins = rand() %10;
    //printf("inserting : %d\n", ins);
    root = insert( ins, root);
  }
  print_ascii_tree(root);
/*
  printf("\nAfter inserting element 5..\n");
  root = insert(500, root);
  print_ascii_tree(root);

  printf("\nAfter inserting element 15..\n");
  root = insert(1509, root);
  print_ascii_tree(root);
  ...
  root = insert(2322, root);
  root = insert(63345, root);
  root = insert(1233, root);
  root = insert(142323, root);
  print_ascii_tree(root);
  ...
  print_ascii_tree(root);
    ...
    printf("\n\nAfter deleting a node (12, root node) with left and right children..\n");
  root = delete(12, root);
  print_ascii_tree(root);
*/
